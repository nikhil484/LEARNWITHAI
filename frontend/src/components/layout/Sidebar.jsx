...
const Sidebar = ({isSidebarOpen,toggleSidebar}) => {
  ...
  <div className='flex items-center justify-between h-16 px-5 border-b border-slate-200/60'>
    <div className='flex items-center gap-3'>
      <div className='flex items-center justify-center w-9 h-9 rounded-xl bg-linear-to-br from bg-emerald-400 to-teal-50 shadow-md shadow-emerald-500/20'>
  ...
  <nav className='flex-1 px-3 py-6 space-y-1.5'>
    {navLinks.map((link)=>{
      ...
      className={({isActive})=>`group flex items-center gap-3 px-4 py-2.5 text-sm font-semibold rounded-xl transition-all duration-200 ${
        isActive ?
        'bg-linear-to-r from-emerald-500 to-teal-500 text-white shadow shadow-emerald-500'
  ...
  <button onClick={handleLogout}
   className='group flex items-center gap-3 w-full px-4 py-2.5 text-sm font-semibold text-slate-700 hover:text-red-600 rounded-xl transition-all duration-200'>
