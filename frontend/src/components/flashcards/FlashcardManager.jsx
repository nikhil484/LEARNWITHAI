...
const handleNextCard = () => {
    if (selectedSet) {
        handleReview(currentCardIndex)
        setCurrentCardIndex(
            (prevIndex) => (prevIndex + 1) % selectedSet.cards.length // ✅ increment for Next
        )
    }
}
...
const handlePrevCard = () => {
    if (selectedSet) {
        handleReview(currentCardIndex)
        setCurrentCardIndex(
            (prevIndex) => (prevIndex - 1 + selectedSet.cards.length) % selectedSet.cards.length // ✅ decrement for Prev
        )
    }
}
    ...
    const handleToggleStar = async (cardId) => {
     try {
        await flashcardService.toogleStar(cardId)
    ...
    const handleConfirmDelete = async () => {
      if(!setSetToDelete) return
      setDeleting(true)
      try {
        await flashcardService.deleteFlashcardSet(setToDelete._id)
        toast.success("Falshcardsets deleted successfully!")
            ...
            return (
                <div className='flex flex-col items-center justify-center py-16 px-6'>
                    ...
                    <p className='text-sm text-slate-500 mb-8 text-center max-w-sm'>
                        Generate falshcards from your document to start and
                ...
                <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4'>
                    {flashcardSets.map((set) => (
                        <div
                            ...
                            className='group relative bg-white/80 backdrop-blur-xl border-2 border-slate-200  hover:border-emerald-300 rounded-2xl px-6 cursor-pointer transition-all duration-200 hover:shadow-lg hover:shadow-emerald-500/10'>
                            <button
    ...
    return (
        <>
            <div className='bg-white/80 backdrop-blur-xl border border-slate-200/50 rounded-3xl shadow-xl  shadow-slate-200/50 p-8'>
            ...
            <Modal
                ...
                <div className='space-y-6'>
                    <p className='text-sm text-slate-600'>
                        ...
                        cannot be undone and all the cards will be permanently removed.
                    </p>
